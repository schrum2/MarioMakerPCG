...
%PY% -m mm2pipeline_data.dataset build --input "%INPUT%" --output_folder "%RAW%" --tileset %TILESET% --convert_to_extended --sliding_window --stride %STRIDE% %META_ARG%
...
if errorlevel 1 goto error
%PY% -m mm2pipeline_data.dataset split --input "%RAW%" --seed %SEED%
if errorlevel 1 goto error
