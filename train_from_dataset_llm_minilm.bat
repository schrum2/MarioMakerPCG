...
setlocal
REM ===========================================================================
REM train_from_dataset_llm_minilm.bat  --  like train_from_dataset.bat but uses
REM LLM-generated captions (MarioMaker_llm_captions.py, Ollama) and a FROZEN
REM MiniLM text encoder instead of a trained MLM. Because MiniLM is pretrained
REM there is no MLM training step; --pretrained_language_model overrides it.
REM   LLM captions -> split -> tokenizer -> train diffusion (MiniLM) -> generate.
REM
REM Requires a running Ollama server with the caption model pulled, e.g.:
REM   ollama pull qwen2.5:14b
REM
REM Usage: train_from_dataset_llm_minilm.bat <dataset.json>
REM ===========================================================================
cd /d "%~dp0"
...
set DIFF_DIR=%OUTDIR%diffusion_llm
set GEN_DIR=%OUTDIR%generated_llm

REM Auto-answer "y" to train_diffusion.py's resume-from-checkpoint prompt.
set YES_FILE=%TEMP%\train_from_dataset_llm_minilm_yes.txt
echo y> "%YES_FILE%"
...
echo.
echo === [3/5] training conditional diffusion (frozen MiniLM; samples every 10, checkpoints every 20) ===
