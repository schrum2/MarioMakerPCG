...
setlocal
REM ===========================================================================
REM train_from_dataset_llm_minilm.bat  --  train text-conditional diffusion on an
REM ALREADY-CAPTIONED dataset.json using a FROZEN MiniLM text encoder. The dataset
REM is assumed to already contain captions, so there is no captioning step and
REM (MiniLM being pretrained) no MLM training step. Flow:
REM   split -> tokenizer -> train diffusion (MiniLM) -> generate.
REM
REM Usage: train_from_dataset_llm_minilm.bat <captioned_dataset.json>
REM ===========================================================================
cd /d "%~dp0"
...
echo.
echo === [2/4] training conditional diffusion (frozen MiniLM; samples every 10, checkpoints every 20) ===
